...
def get_graph_metrics(graph: nx.MultiDiGraph) -> (int, int, set):
...
def get_graph_metrics_by_name(name: str) -> (int, int, set):
...
def generate_and_save_two_cycles_graph(
    first_cycle_num: int, second_cycle_num: int, labels: (str, str), path: str
):
